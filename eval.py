...
#         print('{}.'.format(cxr_labels[i]))
        ...
        # --- 修改开始 ---
        # 1. 确定二值化阈值（这里使用 0.5 作为通用阈值，也可以使用 choose_operating_point 找到的 thresholds）
        optimal_idx = np.argmax(tpr - fpr)
        optimal_threshold = thresholds[optimal_idx] 
        y_pred_binary = (y_pred_i >= optimal_threshold).astype(int)
        
        # 2. 计算 Precision, F1 和 Accuracy
        # 注意：Recall 在第 175 行已经计算好了，变量名是 sens
        from sklearn.metrics import precision_score, f1_score, accuracy_score
        
        prec = precision_score(y_true_i, y_pred_binary, zero_division=0)
        f1 = f1_score(y_true_i, y_pred_binary, zero_division=0)
        acc = accuracy_score(y_true_i, y_pred_binary)
        recall = sens # 官方代码中 sens 就是召回率

        # 3. 将 5 个指标打包进结果
        results = [[roc_auc, prec, recall, f1, acc]]
        df = pd.DataFrame(results, columns=[
            cxr_label+'_auc', 
            cxr_label+'_precision', 
            cxr_label+'_recall', 
            cxr_label+'_f1', 
            cxr_label+'_acc'
        ])
